...
## Summary table of key data sources

Table \@ref(tab:summary-table-data) presents a brief description of the key data sources, their data collection period, update frequency, and major sources of delay that can present challenges for real-time infection estimation. More information on the restricted CDC line list, including details on how often it is updated, can be found at @cdc2020caserestr. The information on the GISAID data is from CoVariants.org [@hodcroft2021covariants]. The seroprevalence surveys are discussed in detail in Section \@ref(sec:serop). Finally, the 2022 publication on reinfection data is referenced as @ruff2022rapid.
...
\hline
\textbf{Data Source}                                                                     & \textbf{Brief description}                                                                                                   & \textbf{Date collection period}                                                                                           & \textbf{\begin{tabular}[c]{@{}l@{}}Updated\\ to public\end{tabular}}                        & \textbf{Major sources of delay}                                                                                                                                                                                                                                                                                                                                                                                                                                                                \\ \hline
\begin{tabular}[c]{@{}l@{}}JHU CSSE \\ Reported Cases\end{tabular}                       & \begin{tabular}[c]{@{}l@{}}Daily new confirmed \\ COVID-19 cases \\ by date of report\end{tabular}                           & Jan. 2020 to Mar. 2023                                                                                                    & Daily                                                                                       & \begin{tabular}[c]{@{}l@{}}Includes delays caused by time- and location-varying \\ reporting to public health, as well as additional delays\\  throughout the pipeline, such as those due to testing, \\ data processing, and reporting backlogs.\end{tabular}                                                                                                                                                                                                                                 \\ \hline
CDC Linelist                                                                             & \begin{tabular}[c]{@{}l@{}}Individual-level data\\ on symptom onset, \\ specimen collection \\ and report dates\end{tabular} & Jan. 2020 to Jul. 2023                                                                                                    & Monthly                                                                                     & \begin{tabular}[c]{@{}l@{}}Similar to JHU CSSE, the time required for the \\ pipeline to process data may vary as jurisdictions \\ collect, process, and report information to the CDC. \\ Additionally, the restricted CDC line list is \\ updated monthly, creating a gap between data \\ collection and update. At the individual \\ case level, data is often initially reported based \\ on lab results, with additional information \\ from patient interviews added later.\end{tabular} \\ \hline
\begin{tabular}[c]{@{}l@{}}GISAID variant \\ proportions \\ from CoVariants\end{tabular} & \begin{tabular}[c]{@{}l@{}}Total number of \\ biweekly sequences \\ categorized by variant \\ group\end{tabular}             & Oct. 2020 to Aug. 2023                                                                                                    & Biweekly                                                                                    & \begin{tabular}[c]{@{}l@{}}Location- and time-specific delays in \\ sequencing, processing, and public availability, \\ with updates typically taking at least two \\ weeks from sample collection to data publication.\end{tabular}                                                                                                                                                                                                                                                           \\ \hline
\begin{tabular}[c]{@{}l@{}}Seroprevalence \\ surveys\end{tabular}                        & \begin{tabular}[c]{@{}l@{}}\% of individuals with\\ at least one past or \\ resolving COVID-19\\ infection\end{tabular}      & \begin{tabular}[c]{@{}l@{}}Aug. 2020 to Dec. 2022 \\ (Commercial);\\ Jul. 2020 to Dec. 2021 \\ (Blood Donor)\end{tabular} & \begin{tabular}[c]{@{}l@{}}Biweekly \\ (Commercial)\\ Monthly;\\ (Blood Donor)\end{tabular} & \begin{tabular}[c]{@{}l@{}}Delays due to the cross-sectional study designs, \\ infrequent and potentially lengthy data collection \\ (e.g., multi-week or monthly), inconsistent\\ data collection. Additional delays due \\ to data collection, processing, and reporting.\end{tabular}                                                                                                                                                                                                       \\ \hline
Reinfection data                                                                         & \begin{tabular}[c]{@{}l@{}}The fraction of \\ infections that \\ are reinfections\end{tabular}                               & Mar. 2020 to Apr. 2022                                                                                                    & October 2022                                                                                & \begin{tabular}[c]{@{}l@{}}Delays in public accessibility of reinfection data, \\ as 2020-2021 data was reported in a 2022\\ publication, and due to the 90-day interval\\  criteria for suspected reinfections. Additional \\ delays would occur if the reinfections were \\ confirmed by genomic sequencing.\end{tabular}                                                                                                                                                                    \\ \hline
\end{tabular}
}
\caption{Overview of data sources for our procedure to estimate COVID-19 infections across all US states, including brief descriptions, the inclusion of relevant uncertainty measures, and major sources of delay.}
\label{tab:summary-table-data}
\end{table}
```
...
(ref:caption) Complete case counts by state in the CDC line list versus the cumulative complete case counts from JHU CSSE as of June 6, 2022. All counts have been scaled by the 2022 state populations as of July 1, 2022 [@uscensus2022annual].
...
estimate of the delay distribution (see Section 2.1)
...
The result of Step 1 (Section 2.1) is $\widehat{x}_{\ell,t}$, case
...
equations given in Section 2.1. Then, we discretize each resulting density
...
similar procedure as described in Section 2.1 with a minor
...
## Details about seroprevalence data {#sec:serop}
...
(presumably unrelated to COVID-19) for the
antibodies to the virus [@bajema2021estimated]. 
The resulting dataset contains seroprevalence estimates
for a number of multi-week collection periods starting in August 2020 to December
2022. 
...
```{r abl-lag-cor, echo=FALSE, out.width = '78%', fig.cap="Lagged Spearman's correlation between the infection and hospitalization rates per 100,000 averaged for each lag across US states and days from early June 2020 to late November 2021, and taken over a rolling window of 61 days. The infection rates are based on the counts for the deconvolved case and infection estimates as well as the reported infections by symptom onset and when the report is symptom onset. Note that each such set of infection counts is subject to a center-aligned 7-day averaging to remove spurious day of the week effects. The dashed lines indicate the lags for which the highest average correlation is attained."}
knitr::include_graphics("adj_unadj_pi_no_inc_hosp_lag_corr_F24.pdf")
```

## Sensitivity analysis of IHRs and CHRs to varying window sizes

The following presents the time-varying IHR and CHR estimates for every state over the period of interest (early June 2020 to late November 2021), with varying window sizes from 15 to 91 days. Note that, the infection, case, and hospitalization counts are subject to a center-aligned 7-day average to mitigate day of the week effects. Additionally, some states may have slightly different starting points due to variations in the availability of the hospitalization data.

```{r ihrs-win-15, echo=FALSE, out.width = '100%', fig.cap="Time-varying IHR and CHR estimates for each state from June 1, 2020 to November 29, 2021, calculated using the correlation-maximizing lags and a 15-day window centered on the date of interest."}
knitr::include_graphics("ihrs-win-15.pdf")
```

```{r ihrs-win-31, echo=FALSE, out.width = '100%', fig.cap="Time-varying IHR and CHR estimates for each state from from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 31-day window centered on the date of interest."}
knitr::include_graphics("ihrs-win-31.pdf")
```

```{r ihrs-win-61, echo=FALSE, out.width = '100%', fig.cap="Time-varying IHR and CHR estimates for each state from from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 61-day window centered on the date of interest."}
knitr::include_graphics("ihrs-win-61.pdf")
```

```{r ihrs-win-91, echo=FALSE, out.width = '100%', fig.cap="Time-varying IHR and CHR estimates for each state from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 91-day window centered on the date of interest."}
knitr::include_graphics("ihrs-win-91.pdf")
```

## Ablation analysis of infection-fatality correlations

The ablation study for infection-fatality correlations follows the same approach as for the infection-hospitalization correlations, described above. Therefore, for details on how the lagged correlation is re-computed using different sets of infection estimates, please refer to the infection-hospitalization ablation study section. The results of this study for infection-fatality correlations are shown in Figure \@ref(fig:abl-lag-cor-deaths), with similar patterns observed as those for hospitalizations in that as we add a subsequent step, the lag time tends to increase. Also notable is that the maximum correlation for deconvolved cases occurs with 3 more days of lag compared to infections. 

```{r abl-lag-cor-deaths, echo=FALSE, out.width = '78%', fig.cap="Lagged Spearman's correlation between the infection and death rates per 100,000 averaged for each lag across US states and days from early June 2020 to late November 2021, and taken over a rolling window of 91 days. The infection rates are based on the counts for the deconvolved case and infection estimates as well as the reported infections by symptom onset and when the report is symptom onset. Note that each such set of infection counts is subject to a center-aligned 7-day averaging to remove spurious day of the week effects. The dashed lines indicate the lags for which the highest average correlation is attained."}
knitr::include_graphics("adj_unadj_pi_no_inc_deaths_lag_corr_F24.pdf")
```

## Sensitivity analysis of IFRs and CFRs to varying window sizes

Figures \@ref(fig:ifrs-win-31) to \@ref(fig:ifrs-win-101) show the time-varying IFR and CFR estimates for every state from early June, 2020 to November, 2021, using varying window sizes from 31 to 101 days. As for hospitalizations, the infection, case, and death counts are smoothed using a center-aligned 7-day average to adjust for day of the week effects. 

```{r ifrs-win-31, echo=FALSE, out.width = '100%', fig.cap="Time-varying IFR and CFR estimates for each state from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 31-day window centered on the date of interest."}
knitr::include_graphics("ifrs-win-31.pdf")
```

```{r ifrs-win-61, echo=FALSE, out.width = '100%', fig.cap="Time-varying IFR and CFR estimates for each state from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 61-day window centered on the date of interest."}
knitr::include_graphics("ifrs-win-61.pdf")
```

```{r ifrs-win-91, echo=FALSE, out.width = '100%', fig.cap="Time-varying IFR and CFR estimates for each state from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 91-day window centered on the date of interest."}
knitr::include_graphics("ifrs-win-91.pdf")
```

```{r ifrs-win-101, echo=FALSE, out.width = '100%', fig.cap="Time-varying IFR and CFR estimates for each state from June 2020 to November 2021, calculated using the correlation-maximizing lags and a 101-day window centered on the date of interest."}
knitr::include_graphics("ifrs-win-101.pdf")
```

## Median and 95% confidence intervals for IHRs and IFRs by state

This presents the median and asymptotic normal 95% confidence intervals of the IHRs and IFRs for each state. While the median IHRs are fairly comparable across states, Montana, Kentucky, New York, Louisiana, Georgia, and Oklahoma have the highest median IHRs, above 0.04. The remaining states' IHRs tend to be between 0.02 and 0.04. For the IFRs, New York has a slightly higher median of around 0.01, compared to most other states having medians closer to 0.005.
 
```{r overall-ihrs-ifrs, echo=FALSE, out.width = '100%', fig.cap="Median and asymptotic normal 95% confidence intervals of the IHRs and IFRs for each state."}
knitr::include_graphics("overall-ihrs-ifrs.pdf")
```
