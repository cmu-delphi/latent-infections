...
adj_df_D21 <- adj_df_D21 %>% select(date, geo_value, case_count_raw, adj_infect) 
adj_df_O21 <- adj_df_O21 %>% select(date, geo_value, case_count_raw, adj_infect) 
adj_df_A21 <- adj_df_A21 %>% select(date, geo_value, case_count_raw, adj_infect) 
...
states_cases_infect_pct_D21 %>%
  ...
  arrange(desc(cases_over_infect_pct)) # 11 states, so in 39 states cases account for < 30% infections
...
# Deconvolved cases
unadj_infect_res[which(unadj_infect_res$cor == max(unadj_infect_res$cor)), ]
```
