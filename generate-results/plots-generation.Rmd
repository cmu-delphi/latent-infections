...
prop_top_circ_df <- read_rds("data/prop_top_circ_df.rds")
state_decon_byvar_df <- read_rds("data/state_decon_byvar_df.rds")

corrs <- read_rds("data/corr_inf_decon_case_lags.rds")
ihrs_dat <- read_rds("data/ihrs_dat.rds")
density_data <- read_rds("data/gamma_density_data.rds")
ca_interp_list <- read_rds("data/ca_interp_list.rds")
...
ggplot(adj_df) +
  ...
  geom_line(aes(date, unadj_inf_rate, color = "Deconvolved cases"), size = 0.4, alpha = 0.8) +
  ...
  scale_color_manual(name='', # Legend
                     ...
                     values=c('Infections' = "midnightblue",
                              'Deconvolved cases' = "springgreen4",
...
six_plt1 <- ggplot(adj_df %>% filter(geo_value %in% six_states)) +
  geom_rect(data = prop_top_circ_df %>% 
              ...
              rename("geo_value" = "State", Variant = variant), 
  ...
  scale_color_manual(
    name='', # Legend
    breaks=c('Infections', 'Deconvolved cases', 'Reported cases'),
    values=c('Infections' = "midnightblue",
             'Deconvolved cases' = "springgreen4",
             'Reported cases' = "darkorange2")
  ) +
  scale_fill_manual(
    ...
    guide = guide_legend(direction = "horizontal", ncol = 3)
  ) +
  ylab("") +
...
corrs_df <- list_rbind(corrs[1:3], names_to = "type") |> 
  mutate(type = case_when(
    type == "infect_res" ~ "Infections",
    type == "cases_res" ~ "Reported cases",
    type == "unadj_infect_res" ~ "Deconvolved cases"
  ))
