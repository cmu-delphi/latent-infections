# EmpshrinkDelayDistByState.py for pos_spec_dt and cdc_report_dt
...
overall_emp_dist = pickle.load(open(f'{naive_dir}/uncensored_empirical_distribution_d60c_feb8_pr.p', 'rb'))
...
for state in states:
    ...
    for run_date in tqdm(date_range(first_data_date, end_date)):
        ...
        if len(delay_df) > 0: # if more than 0 rows in delay_df
            ...
            emp_dist = delay_df.groupby('report_delay').pos_spec_dt.count()
            emp_dist = emp_dist.reindex(support, fill_value=0)
            emp_dist /= emp_dist.sum()
        else:
            emp_dist = 0

        # num of rows/records for the state in the linelist with pos_spec_dt in [t-1.25*60 + 2, t + 60]
        num_records_delay_df = len(delay_df)

        # num of JHU reported cases in the state over roughly centered time window
    ...
    pickle.dump(fully_observed_pmfs, open(f'{storage_dir}/{state}/{state}_Empshrink_delay_distribution_d60c_feb8_pr.p', 'wb'))
    pickle.dump(shape, open(f'{storage_dir}/{state}/{state}_Empshrink_shape_d60c_feb8_pr.p', 'wb'))
    pickle.dump(scale, open(f'{storage_dir}/{state}/{state}_Empshrink_scale_d60c_feb8_pr.p', 'wb'))
    pickle.dump(mu, open(f'{storage_dir}/{state}/{state}_Empshrink_mu_d60c_feb8_pr.p', 'wb'))
    pickle.dump(var, open(f'{storage_dir}/{state}/{state}_Empshrink_var_d60c_feb8_pr.p', 'wb'))
