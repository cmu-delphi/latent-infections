...
# Remove last sero for each state (save ND which ends in 2020) from commercial dataset
last_ND_comm = comm_sero_df %>% group_by(res_state) %>% filter(res_state == "ND") %>% slice(n())
comm_sero_df = comm_sero_df %>% group_by(res_state) %>% slice(1:(n()-1))
comm_sero_df = bind_rows(comm_sero_df, last_ND_comm) %>% arrange(res_state, Date)
...
# Try A. approach to aggregate these into a single date per month - the median of these median dates

blood_cdc_df$res_state <- str_extract(blood_cdc_df$Region, "[^-]+")

# Make sure Date is in correct format and treated as a Date obj
...
setwd("/Users/admin/Downloads")
saveRDS(sero, "sero_for_ssmod_Nov5.Rds")
