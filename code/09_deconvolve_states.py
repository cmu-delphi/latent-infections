...
#######################################################################################################################
for state in states:
    # Load convolution pmfs
    convol_storage_dir = '/Users/admin/Downloads/convolution/' + str(state)

    conv_pmfs = pickle.load(open(f'{convol_storage_dir}/{state}_conv_distribution_d60c_nov5.p', 'rb'))
    ...
    C, _ = construct_day_specific_convolution_matrix(y, run_date, conv_pmfs)
    ...
    np.save(f'{storage_dir}/{state}/{state}_deconvolution_res_d60c_nov5.npy', res)

    # Save a plot comparing the tf results to the jhu incidence counts
    ...
    plt.plot(conv_pmfs.keys(), res, ls='--', label='Est. incident infections')
    plt.xlabel('Time')
    plt.ylabel('New infections in ' + state)
    plt.legend(loc='upper left')
    plt.show()
    plt.savefig(storage_dir + "/" + str(state) + "/" + str(state) + "_deconvolution_plot_d60c_nov5_w_7dav.png")