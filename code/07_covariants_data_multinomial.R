...
cav <- read_csv("seq_df_filled_Nov5.csv")
...
for(state in state.abb){
  ...
  if(state == "CA") ggsave(filename = "proportions_before_multi.pdf")
  
  ca_interp_list <- list(date = date, y_props = y_props, pred2 = pred2)
  saveRDS(ca_interp_list, "ca_interp_list.rds")
  ...
  if(state == "CA") ggsave(filename = "proportions_after_multi_pred.pdf")
  ...
  rows_prior_to_may11 <- do.call("rbind", replicate(n_rows, c("Alpha" = 0, "Beta" = 0, "Epsilon" = 0, "Iota" = 0, "Gamma" = 0, "Delta" = 0, "Omicron" = 0, "Other" = 1), simplify = FALSE))
  ...
  final_df = rbind(final_df, state_res)
}
...
# Save df of results
write_csv(final_df, "seq_df_post_decon_j18.csv")
