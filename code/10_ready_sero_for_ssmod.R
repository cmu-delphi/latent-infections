...
comm_sero2_sub <- comm_sero2 %>%
  select(c(Site, n, Rate, lb, ub, midpoint_date)) %>%
  rename(res_state = Site, Date = midpoint_date) %>%
  arrange(Date)
...
comm_sero_df <- comm_sero2_sub %>%
  ...
  mutate(
    ...
    weights = weights / sum(weights) * length(Rate)
  ) # re-scale weights so they sum to the sample size like glmnet does
...
sero_comm_bd_mean_w_ratio <- sero %>%
  filter(!is.na(mean_obs_weights)) %>%
  group_by(res_state, Source) %>%
  slice(1) %>%
  group_by(res_state) %>%
  arrange(desc(Source)) %>%
  summarise(mean_w_ratio = first(mean_obs_weights) / last(mean_obs_weights))
...
sero <- sero %>%
  group_by(res_state) %>%
  mutate(
    weights = ifelse(mean_w_ratio > 1 & Source == "Commercial", weights * mean_w_ratio, weights), # len_ratio > 1 means more comm than blood donor measurements
    weights = ifelse(mean_w_ratio < 1 & Source == "Commercial", weights * mean_w_ratio, weights)
  ) # len_ratio < 1 means less comm than blood donor measurements

saveRDS(sero, here("data", "ww_sero_for_ssmod_J18.Rds"))
