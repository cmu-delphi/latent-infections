...
adjusted_infections_fun <- function(state, inv_ratios_all_states, max_y_val){
  ...
  ggsave(filename = here("gfx", "state", paste0(state, "_inv_rr_after_const_imp_F24.png")))
  ...
  state_df_of_res = state_df_of_res %>%
    mutate(adj_inf_rate = (adj_infect / res_state$pop) * 100000,
  ...
  state_df_of_res = state_df_of_res %>%
    mutate(fifty_lowb_rate = (fifty_lowb / res_state$pop) * 100000,
  ...
  if (max(state_df_of_res$adj_inf_rate) < 1000){
    max_y_val = (max(state_df_of_res$adj_inf_rate) +
                   1.5*IQR(state_df_of_res$adj_inf_rate))
  } else{
    max_y_val = 1000
  }
  ...
  ggsave(filename = here("gfx", state, paste0(state, "_plot_adj_unadj_infect_jhu_no_cases_F24.png")))
  ...
  ggplot(state_df_of_res) +
    ...
    theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 10),
          ...
          plot.title = element_text(size = 12, face = "bold", hjust = 0.5))
  ggsave(filename = here("gfx", state, paste0(state, "_plot_adj_unadj_infect_jhu_cases_F24.png"), width = 9, height = 7))
...
inv_ratios_all_states = readRDS(here("data", "inv_ratios_all_states_F24.RDS"))

# Load in proportion of circulating variants
prop_circ_df <- read_rds(here("data", "seq_prop_df.rds"))
...
saveRDS(prop_top_circ_df, here("data", "prop_top_circ_df.rds"))
...
saveRDS(adj_df_list, file = here("data", "adj_df_list_F24.RDS"))
...
ggsave(filename = here("gfx", "state_nia_est_faceted_F24.pdf"), width = 12, height = 7)
...
ggsave(filename = here("gfx", "state_niauc_est_6states_F24.pdf"), width = 12, height = 7)

# Estimates of the deconvolved cases by variant
state_decon_byvar_df = c()
for(state in six_states){
  st_path <- here("data", "state")

  state_decon_byvar <- read_rds(here(st_path, "final-thetas-df.rds")) %>%
...
saveRDS(state_decon_byvar_df, here("data", "state_decon_byvar_df.rds"))
...
ggsave(filename = here("gfx", "state_decon_byvar_est_6states_F24.pdf"), width = 12, height = 7)
...
ggplot(adj_df) +
  ...
  theme(axis.text.x = element_text(hjust = 0.1, vjust = 3, size = 5.5),
        ...
        panel.spacing = unit(3, "pt"),
        strip.text = element_text(size = 4, face = "bold", margin = margin(1, 0, 1, 0, "mm")))
ggsave(filename = here("gfx", "state_niauc_est_faceted_F24.pdf"), width = 12, height = 7)
...
ggsave(filename = here("gfx", "choro_inf_case_rates_F24.pdf"), width = 7, height = 4)
...
states_cases_infect_pct_D21 %>%
  ...
  select(-c(sum_case_count_raw, sum_adj_infect, sum_50_lb, sum_50_ub, sum_80_lb, sum_80_ub, sum_95_lb, sum_95_ub))
