...
vdc_path <- here("code", "supporting_files_for_deconvolutions_by_state")
Rcpp::sourceCpp(here(vdc_path, "src", "estim_path.cpp"))
source(here(vdc_path, "utils-arg.R"))
source(here(vdc_path, "utils-enlist.R"))
source(here(vdc_path, "cv_estimate_tf.R"))
...
for (state in state.abb) {
  ## These data are all missing
  path_naive <- here("data", "naive_delay_distributions", state)
  state_op_list <- pd$read_pickle(
    paste0(path_naive, "_Empshrink_delay_distribution_feb8_op.p")
  )
  # don't use {reshape}, this package is deprecated.
  # Use `dplyr::pivot_longer()`
  ...
  ## This data is missing. Adjust path.
  vmix <- read_rds(here("data", "seq_prop_df.rds")) %>%
    filter(State == state)
  ...
  write_rds(listy, here("data", state, "convolution-mat-list.rds"))
  ...
  state_pr_list <- pd$read_pickle(
    paste0(path_naive, "_Empshrink_delay_distribution_d60c_feb8_pr.p"))
  ...
  if(ablation == TRUE){
    ...
    write_rds(final_thetas_pr, here("data", state, "final-thetas-pr.rds"))
    ...
    # By variant
    params <- read_rds(here("data", state, "convolution-mat-list.rds"))
    ...
    for(var in params$Variant){
      ...
      final_thetas_sp_list[[var]] = data.frame(
        time_value = seq(start_date, end_date, by = "day"),
        geo_value = state,
        infect = final_thetas_sp
      )
    }
    final_thetas_sp_df_state = bind_rows(final_thetas_sp_list, .id="variant")


    write_rds(final_thetas_sp_df_state,
              here("data", state, "final-thetas-sp-df.rds"))
  }
  ...
  write_rds(final_thetas_op_df_state, here("data", state, "final-thetas-df.rds"))
  ...
  ggsave(filename = here("data", state, paste0(state, "infect_by_variant.png")))
  ...
  ggplot(final_thetas_op_df_state_sub, aes(time_value, infect, fill = variant)) +
    ...
    ylab("Infections") +
    scale_fill_discrete(name = "Variant")
  ggsave(filename = here("data", state, paste0(state, "infect_by_variant_end_", end_date, ".png")))
}
