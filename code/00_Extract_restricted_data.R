...
for(i in 1:ncsv){
  ...
  data.tmp[[i]] = data.tmp[[i]] %>% select("res_state", "res_county", "onset_dt", "cdc_report_dt", "pos_spec_dt")
  ...
  data.tmp[[i]] = data.tmp[[i]] %>% filter((!is.na(onset_dt) & !is.na(pos_spec_dt)) | (!is.na(pos_spec_dt) & !is.na(cdc_report_dt)))
}
...
save(cdc_dataset, file = "cdc_restricted_dataset_Feb8.RData")
write.csv(cdc_dataset,"cdc_restricted_dataset_Feb8.csv",na="NA",row.names=FALSE)
