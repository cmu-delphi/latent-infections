...
if(any(fixed) == TRUE) {
  o <- optim(inits[!fixed], fit_my_ssm_w_fixed, fixed_par = inits[fixed], fixed = fixed, model = ss, updatefn = model_builder, k = k)
  inits_nonfix <- o$par
  inits[!fixed] = inits_nonfix
  par <- c(logistic(inits[1]), reals_to_nonneg(inits[2:4]))
} else {
  o <- optim(inits, fit_my_ssm, model = ss, updatefn = model_builder, k = k)
  inits <- o$par
  par <- c(logistic(inits[1]), reals_to_nonneg(inits[2:4]))
}
ss <- model_builder(par, k = k)
