...
deconv_cases <- read_rds(here("data", "CA-final-thetas-pr.rds"))
deconv_cases <- tibble(
...
dat <- adj_df |>
  ...
  filter(geo_value == "CA", time_value >= ymd("2020-06-01"), 
         time_value <= ymd("2021-03-01")) |>
...
s2 <- dat |>
  ...
  geom_line(aes(y = Epsilon), colour = "mediumpurple", linewidth = 1) + 
  scale_x_date(expand = expansion()) + 
  scale_y_continuous(expand = expansion(), breaks = 1:5 * 25) +
  coord_cartesian(xlim = ymd(c("2020-06-01","2021-02-01")), ylim = c(0, 125)) +
...
s3 <- dat |>
  ...
  geom_line(aes(y = Epsilon), colour = "grey75", linewidth = 1) + 
...
s4 <- dat |> 
  ...
  geom_line(aes(y = Epsilon), colour = "grey90", linewidth = 1) + 
