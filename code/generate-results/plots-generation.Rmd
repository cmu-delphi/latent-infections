...
corrs_hosp <- read_rds(here("data", "inf_case_corrs_hosp.rds"))
corrs_deaths <- read_rds(here("data", "inf_case_corrs_deaths.rds"))
ihrs_dat <- read_rds(here("data", "ihrs_dat.rds"))
ifrs_dat <- read_rds(here("data", "ifrs_dat.rds"))
...
ggplot(adj_df) +
  ...
  theme_bw(16) +
  theme(axis.text.x = element_text(hjust = 0.1, size = 8),
...
p1 <- plot_usmap_fun(first_date, "cases_rate_7d_av", "Reported cases", max_choro_lim_cases) 
p2 <- plot_usmap_fun(second_date, "cases_rate_7d_av", "Reported cases", max_choro_lim_cases) 
p3 <- plot_usmap_fun(third_date, "cases_rate_7d_av", "Reported cases", max_choro_lim_cases) 
p4 <- plot_usmap_fun(fourth_date, "cases_rate_7d_av", "Reported cases", max_choro_lim_cases) 
p5 <- plot_usmap_fun(fifth_date, "cases_rate_7d_av", "Reported cases", max_choro_lim_cases) 

# Adjusted infection rates plots
p6 <- plot_usmap_fun(first_date, "adj_inf_rate", "Infections", max_choro_lim_inf) 
p7 <- plot_usmap_fun(second_date, "adj_inf_rate", "Infections", max_choro_lim_inf) 
p8 <- plot_usmap_fun(third_date, "adj_inf_rate", "Infections", max_choro_lim_inf) 
p9 <- plot_usmap_fun(fourth_date, "adj_inf_rate", "Infections", max_choro_lim_inf) 
p10 <- plot_usmap_fun(fifth_date, "adj_inf_rate", "Infections", max_choro_lim_inf) 
...
six_plt1 <- ggplot(adj_df %>% filter(geo_value %in% six_states)) +
  ...
  scale_y_continuous(labels = scales::comma, limits = c(0, 700), expand = c(0,0)) + 
  scale_color_manual(
    name='', # Legend
...
```{r corr-plot, include = FALSE, fig.width=6, fig.height=4}
# Infection and case average correlation with hospitalizations across lags on the same plot
corrs_df <- list_rbind(corrs_hosp[1:2], names_to = "type") |> 
  mutate(type = case_when(
    type == "infect_res" ~ "Infections",
    type == "cases_res" ~ "Reported cases"
  ))
corr_plt1 <- ggplot(corrs_df, aes(lag, cor, color = type, group = type)) +
  geom_line() + 
  geom_point() +
  geom_vline(
  ...
  labs(x = "Days before hospitalization", y = "Average correlation") +
  theme_bw(16) +
...
ggplot(ihrs_dat, aes(x = time_value)) +
  ...
  scale_color_manual(
    name = "",
    values=c('IHR' = "midnightblue",'CHR' = "darkorange2")
    )
```

```{r corr-plot-deaths, include = FALSE, fig.width=6, fig.height=4}
# Infection and case average correlation with deaths across lags on the same plot
corrs_df_deaths <- list_rbind(corrs_deaths[1:2], names_to = "type") |> 
  mutate(type = case_when(
    type == "infect_res" ~ "Infections",
    type == "cases_res" ~ "Reported cases"
  ))
corr_plt2 <- ggplot(corrs_df_deaths, aes(lag, cor, color = type, group = type)) +
  geom_line() + 
  geom_point() +
  geom_vline(
    data = filter(corrs_df_deaths, cor == max(cor), .by = type),
    aes(xintercept = lag, color = type), linetype = 2) +
  scale_color_manual(
    name = "", 
    values = c("Infections" = "midnightblue", 
...
```{r ifrs, include = FALSE, fig.width=12, fig.height=7}
# Rolling window time-varying IFRs for each state 

ggplot(ifrs_dat, aes(x = time_value)) +
  geom_line(aes(y = IFR, color = "IFR")) +
  geom_line(aes(y = CFR, color = "CFR")) +
  facet_geo( ~ geo_value, grid = my_grid) +
  ylab("Death to incidence ratio") +
  scale_x_date(name = "", breaks = seq(as.Date("2020-07-01"), as.Date("2021-07-01"), by = "6 months"),
               date_labels = "%m/%y", expand = c(0,0)) +
  ...
  scale_color_manual(
    name = "",
    values=c('IFR' = "midnightblue",'CFR' = "darkorange2")
  )
```

```{r corr-plot-stacked, include = FALSE, fig.width=13, fig.height=4}
# Infection and case average correlation with hospitalizations and deaths across lags in two side-by-side plots

pg <- plot_grid(corr_plt1, corr_plt2, nrow = 1, align = "v", labels = "AUTO")
pg
```
