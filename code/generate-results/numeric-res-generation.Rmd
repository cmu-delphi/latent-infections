...
mean(case_date_vec_other - inf_date_vec_other)

# 2. Delta
inf_top_delta = adj_df_D21 %>% 
  filter(geo_value %in% six_states) %>% 
  group_by(geo_value) %>% 
  arrange(desc(adj_inf_rate)) %>% 
  slice(1) %>% 
...
mean(case_date_vec_delta - inf_date_vec_delta)
```
