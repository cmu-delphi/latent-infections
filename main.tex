...
% Later in the pandemic, the fall of 2021, we can see that the major Delta wave is only faintly detectable from cases in a number of eastern states such as Maryland and Connecticut. It is clear that cases fail to adequately capture the rise and fall in infections during this time.
...
rates, in that case rates tend to underrepresent infections to a great extent.  %% Maybe this sentence and the next few belong more in Section 2.2
...
\right )^2 + \lambda \sum_{t \geq 5}\bigl| x_t - 4x_{t-1} +  6x_{t-2}  - 4x_{t-3}  + x_{t-4}\bigr|. %\lambda \|D^{(4)}x\|_1. 
\end{align*}
...
%Possible problem here is that reinfections not include third infections (see comments
...
% Furthermore, this end date is enforced by the forthcoming model 
% and the subsequent extrapolation procedure, which is detailed in Supplementary Materials \ref{supp:ssapm}).
...
%\bibliographystyle{naturemag} %% Make sure references in line with
%requirements for Nature Communications articles (see pg 3 from here:
...
\section{Additional information about data or estimation methodology}
...
\end{figure} %% Maybe move this figure location back to under Ablation subsection
