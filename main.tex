...
\centering
    ...
    July 1, 2022 from \citep{uscensus2022annual}.}
    \label{fig:prop_cc_cdc_vs_jhu}
\end{figure}

In the line list, we observe unusual and striking spikes in reporting in 2020
in comparison to 2021. When we plot by report date, we find that a few states are
...
interquartile range and retain only those that exceed the upper bound as potential
...
first two bins, and then lower at $500$ for all remaining bins. A similar trial and error
...
fall of 2020 and 2021. For example, take the Alpha wave in 
...
where about $9.0\%$ (95\% confidence interval: $[3.9, 100]$) of the infections were reported over January 2022. 
This was followed by Arizona with $12.4\%$ $[9.4, 97.9]$, and Minnesota with $13.9\%$  $[8.0, 53.4]$. 
More broadly, in $27$ states
...
\thispagestyle{empty}
\begin{figure}[!tb]
     \hspace*{-1.5cm}\includegraphics[width=1.55\textwidth]{state_niauc_before_dec11.pdf} 
...
\begin{figure}[!tb]
\centering
     \hspace*{-1.5cm}\includegraphics[width=1.07\textwidth]{state_niauc_est_6states.pdf} 
    \caption{Estimates of the number of daily new infections per 
...
time (\autoref{fig:state_est_upto_dec1121}, \autoref{fig:state_est_after_dec1121},
 \autoref{fig:six_state_est}, \autoref{fig:chloro_inf_rates}). 
