...
ratios (IHRs) for each state and compare those to similarly derived case-hospitalization ratios (CHRs).
...
% Consider changing time period of consideration. Currently from June 1, 2020 
% to January 31, 2022, but could extend to some extent in either direction.
% March 1, 2020 to March 1, 2022 would be ideal.
...
Note that we already acquired the new number of reported infections from our
...
enforce a center-aligned rolling window of $61$ days for each such computation.
...
(ie. the deconvolved case estimates); 4. those from applying the leaky model to produce
estimates for both the reported and the unreported cases (ie. the infection estimates).
...
the CHRs that were computed using the optimal lag for cases and hospitalizations for comparison.
(\autoref{fig:IHR_7dav}). 

\begin{figure}[!tb]
\centering
    \includegraphics[width=.8\textwidth]{infect_case_hosp_lag_corr_Oct2.pdf} 
    \caption{Lagged Spearman's correlation between infection and hospitalization rates per $100,000$ as
    well as between case and hospitalization rates per $100,000$. The averages shown are for each lag, across
     US states and days over June 1, 2020 to January 31, 2022, and taken over a rolling window of $61$ days. 
     Note that the infections, cases, and hospitalization counts are subject 
...
\centering
    ...
    \caption{Lagged Spearman's correlation between the infection and hospitalization rates per $100,000$
    averaged for each lag across US states and days over June 1, 2020 to January 31, 2022, 
    and taken over a rolling window of $61$ days.
    The infection rates are based on the counts for the deconvolved case and infection estimates as 
...
\thispagestyle{empty}
\begin{figure}[!tb]
   \hspace*{-1.5cm}\includegraphics[width=1.5\textwidth]{IHR_7dav_Oct23.pdf} 
    \caption{Time-varying IHR and CHR estimates for each state from June 1, 2020 to 
    January 31, 2022, obtained using the corresponding optimal lag from the systematic lag analysis. 
    Note that the infection, case, and hospitalization counts are subject to a 
...
\begin{figure}[!tb]
\centering
     \hspace*{-1.5cm}\includegraphics[width=1.07\textwidth]{state_niauc_est_6states_Oct25.pdf} 
    \caption{Estimates of the number of daily new infections per 
    $100,000$ for a sample of six US states from June 1, 2020 to January 31, 2022 (dark blue 
    line). The blue shaded regions depict the 50, 80, and 95\% confidence intervals. The
    background is shaded to indicate the top variant in circulation that is at least
    $60\%$ of all variants in circulation at the time.}
    \label{fig:six_state_niauc_est_faceted}
\end{figure}
...
time (\autoref{fig:state_niauc_est_faceted}, \autoref{supp:state_niauc_est_faceted_log10}, 
\autoref{fig:six_state_niauc_est_faceted}, \autoref{fig:chloro_inf_rates}). 
...
While we computed IHRs for all states, it is important to note that
 the IHR is also likely to vary within
...
\maketitle
\section{Figures}  
%\subsection{Estimates of infections per 100,000 using a base-10 log scale on the y-axis}
...
\begin{figure}[!tb]
    ...
    \caption{Estimates of the number of daily new infections per
     $100,000$ population for each US state from June 1, 2020 to January 31, 2022
      ...
      The blue shaded regions depict the 50, 80, and 95\% confidence 
      intervals for the estimates, while the teal line represents the 
      number of new daily new deconvolved cases per $100,000$, and the dotted 
      orange line represents the 7-day average of the new cases per $100,000$ as 
      of the same date.}
