...
(as depicted in \citealt{hodcroft2021covariants}).
...
\citealt{dong2020interactive}). From the same API, we also retrieve the daily
...
management (presumably unrelated to COVID-19, \citealt{bajema2021estimated}). The
...
(which is reasonable based on \citealt{zaki2021estimations} and
\citealt{cortes2022sars}).
...
them according to onset (as in \citealt{jahja2022real}):
\begin{align*}
...
\begin{align}
\minimize_{a, \gamma}\ \frac{1}{2}\sum_{t \in T}w_t\left (s_t - (1 -\gamma)s_{t-1} 
- a_t\Delta R_t n_t  \right )^2 + \frac{\lambda}{2} \|D^{(3)}a\|_2^2 \label{eq:leakypr}
\end{align}
where $D^{(3)}$ is the discrete derivative matrix of order $3$, $\lambda$ is the
tuning parameter, and $\gamma$ is the percentage of people who lose immunity
between time $t$ and time $t+1$. 

\attn{Do we need the previous equation / setup? Can we just present the 
state-space version?}
...
Gaussian state space model (as in \citealt{durbin2012time, helske2017kfas}).
...
time series and (refer to \citealt{durbin2012time} for further details).
...
reporting delay data is used (i.e., excluding the incubation period data); 3. those from
the deconvolution procedure that utilizes both the incubation period and delay data 
(i.e., the deconvolved case estimates); 4. those from applying the leaky model to produce
estimates for both the reported and the unreported cases (i.e., the infection estimates).
...
fewer than $50\%$ of infections were reported. Only $10$ states of Florida, New York, Colorado, New Jersey, 
...
\begin{figure}[!tb]
    \centering
    \includegraphics[width=.99\linewidth]{state_niauc_est_faceted_Oct2.pdf} 
...
\begin{figure}[!tb]
\centering
    \includegraphics[width=.9\linewidth]{state_niauc_est_6states_Oct25.pdf}
...
By reconstructing the time series of COVID-19 infections per 100,000 population
for each US state from June 1, 2020 to January 31, 2022, we observe rates of
infections that vary in intensity and disease burden across space and time
(\autoref{fig:state_niauc_est_faceted},
\autoref{supp:state_niauc_est_faceted_log10},
\autoref{fig:six_state_niauc_est_faceted}, \autoref{fig:chloro_inf_rates}). The
...
factors (such as healthcare performance) which lead to lower infection rates.
...
state for June 1, 2020 to January 31, 2022, which we will now discuss in more
depth and with respect to others' findings. While all states present waves that
...
lack of health insurance and the fear of having to pay costly out-of-pocket
