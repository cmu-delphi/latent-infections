...
\attn{Perhaps plot the incubation periods? Maybe sharing a panel with the 
circulation proportions below.}
...
\end{align}
\attn{These summs aren't quite right: the outer $t$ makes $x_{t-k}$ run
negative? Actually this wouldn't be a problem, except that we don't deconvolve
before $t=1$, we renormalize the delays. Perhaps too much detail, but the math
annoys me. Ideas?}
...
infection and hospitalization rates for each observed between June 1, 2020
to November 29, 2021 with a center-aligned rolling window of 61 days.
We then average these correlations across all
states and times for each lag. 
...
who were infected on the lagged number of days before. \attn{Isn't there a
window here?}
