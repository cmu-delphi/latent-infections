...
In what follows, we describe how we estimate the daily incident
infections for each \US state from June 1, 2020 to November 29, 2021.
...
time-varying delays from positive specimen to report date 
and use them to deconvolve reported cases to their collection date.
Next, we estimate the delay from symptoms to positive specimen,
combine this with variant specific infection-to-symptoms delays, and
push back to the date of infection. These reported infection
estimates are aggregated across the variant categories and adjusted
for the unreported infections by using seroprevalence survey data.
...
%line list data on COVID-19 cases from the CDC \citep{cdc2020caserestr}. 
...
was used to estimate the necessary delay distribution. Together,
...
that cases with positive specimen collection on time $t-k$ are reported at $t$. 
\end{linenomath*} 
\begin{linenomath*} 
Then, we
...
$k$, we use the CDC line list \citep{cdc2020caserestr}. 
The line list contains three key dates of interest for many
cases that eventually appear in case reports: symptom onset,
positive specimen collection, and report to the CDC. Handling missingness
in these dates requires careful attention, and so such details are presented in
 Supplementary Methods \Cref{supp:linelist-details}.
...
distribution $\pi_{\ell,t}(k)$. Additional minor details are deferred to Supplementary Methods
...
defer details to Supplementary Methods \Cref{sec:variant-proportions}. The resulting
...
be reported to public health. Because this deconvolution is performed separately for
...
deconvolved case estimates by the inverse reporting ratio, which is
the ratio of the true number of new infections to
...
\begin{linenomath*}
We use two major seroprevalence surveys to estimate the proportion of the
population with evidence of previous infection in each state over time  % the 2020--2021 Blood Donor Seroprevalence Survey and the Nationwide Commercial Lab Seroprevalence Survey 
\citep{cdc2021blood, cdc2021comm}. Details on these surveys are presented in Supplementary
Methods \Cref{supp:sero-details}. 
...
data, extrapolation of the latent quantities beyond the
observed seroprevalence period, and maximum likelihood
...
in Supplementary Methods \Cref{supp:ablation}.
