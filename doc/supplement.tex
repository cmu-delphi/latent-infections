...
\Cref{tab:order-events-table} presents the percent of pairwise occurrences
...
\Cref{fig:prop-cc} suggests that this issue impacts states
...
each state is shown in \Cref{fig:prop-cc}, from which it is clear
...
\attn{Rachel, note the use of booktabs commands so this looks OK. See, e.g., \url{https://people.inf.ethz.ch/markusp/teaching/guides/guide-tables.pdf}}
\begin{table}[h!]
    \centering
...
\attn{Rachel: does this notation all match the manuscript?}
...
\attn{Rachel: some text that was in methods} Estimating the delay from symptom
onset to positive specimen date follows the same procedure with a few minor
adjustments. First, we allow $k$ to range from $-3$ to $21$ (rather than 1 to
60). These upper and lower bounds are based on the largest delay values for the
state-wide 0.05 and 0.95 quantiles. This is reasonable because the median delay
is very short at approximately 2 days, and an asymptomatic individual may test
positive following a known exposure, before the onset of symptoms.
...
distribution, we solve the same moment equations described in \attn{described where?}
\Cref{sec:delaystop}. Then, we discretize each resulting density shown in \Cref{fig:inc_gammas} to the
...
timing of the data points for each state (\Cref{fig:sero-blood-comm-compar}).
...
\attn{Rachel: Please take a stab at revising/simplifying this whole section.These next paragraphs were removed from the manuscript, so you can use some of it here if needed.}
...
deferred to Supplementary Methods \Cref{supp:ssapm}.
